...
function ComparePlants() {
    ...
    // ottimizzazione della lista comparativa con useMemo
    const compareList = useMemo(() => {

        return plants.map((p, index) => <PlantCardDetail key={p.id ?? index}
            data={p}
    ...
    }, [plants, favorites, toggleFavorite]);
    ...
    return (
        <div>
            ...
            <div className={`compare-list-container ${plants.length === 4 ? 'four-compare-items' : ''}`}>
                {plants.length > 0 ? compareList : <p>Comparatore vuoto!</p>}
            </div>
